...
function normalizeSegmentationRows(rows) {
    ...
    for (const row of rows) {
        ...
        if (category && value !== null) {
            feature.categorical_values.push({
...
async function handleSegmentAICopy(reqUrl, res) {
    try {
        const parsedUrl = new URL(reqUrl ?? '/', 'http://localhost');
        ...
        if (!forceRefresh) {
            const cached = getCache(cacheKey);
            if (cached) {
                sendJson(res, 200, { ...cached, cache: 'hit' });
                return;
            }
        }
    ...
    `;
        ...
        setCache(cacheKey, result);
        sendJson(res, 200, { ...result, cache: forceRefresh ? 'refresh' : 'miss' });
    } catch (error) {
        sendJson(res, 500, {
...
const server = createServer(async (req, res) => {
    ...
    if (pathname === '/api/segment-ai-copy') {
        if (method !== 'GET') {
            sendJson(res, 405, { error: 'Method not allowed' });
            return;
        }
